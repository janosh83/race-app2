...
export default function CheckpointList({ checkpoints = [], onRemove = () => {}, raceId = null, onImported = () => {}, onUpdate = () => {}, supportedLanguages = [] }) {
  ...
  const handleEdit = (checkpoint) => {
    ...
    setEditForm({
      ...
      numOfPoints: checkpoint.num_of_points ?? 0
    });
  };
  ...
  const validateItem = (it) => {
    // minimal validation: title present
    if (!it || typeof it !== 'object') return t('admin.checkpoints.validationItemNotObject');
    if (!it.title) return t('admin.checkpoints.validationMissingTitle');
    return null;
  };
  ...
  const handleImport = async (e) => {
    ...
    setImporting(true);
    try {
      // Build strict canonical payload (all in one request)
                  ...
                  <div className="d-flex justify-content-between align-items-start">
                    <div className="flex-grow-1" style={{ cursor: 'pointer' }} onClick={() => setExpandedId(isExpanded ? null : cpId)}>
                      <div className="d-flex align-items-center gap-2">
                        ...
                        <span className="badge bg-secondary">{cp.num_of_points ?? 0} {t('admin.common.pointsShort')}</span>
                      </div>
                    </div>
                      ...
                      <div className="row">
                        <div className="col-md-6">
                          ...
                          <dl className="row small">
                            ...
                            <dd className="col-sm-8">{cp.num_of_points ?? 0}</dd>
                          </dl>
                        </div>
      ...
      <div className="card card-body">
        <form onSubmit={handleImport}>
          <div className="d-flex align-items-center gap-2 mb-2">
            <label className="form-label mb-0">{t('admin.checkpoints.importTitle')}</label>
            <button
              type="button"
              className="btn btn-sm btn-link p-0"
              onClick={() => setShowHelpModal(true)}
              title={t('admin.checkpoints.importHelpTitle')}
            >
      ...
      {showHelpModal && (
        <div className="modal d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{t('admin.checkpoints.importHelpTitle')}</h5>
                <button
                  type="button"
                  className="btn-close"
              ...
              <div className="modal-body">
                ...
                <ul>
                  ...
                  <li><strong>description</strong> {t('admin.checkpoints.importHelpFieldDesc')}</li>
                  <li><strong>latitude</strong> {t('admin.checkpoints.importHelpFieldLat')}</li>
                  <li><strong>longitude</strong> {t('admin.checkpoints.importHelpFieldLng')}</li>
                  <li><strong>num_of_points</strong> {t('admin.checkpoints.importHelpFieldPoints')}</li>
  ...
  {
    "title": "Main Square",
    "description": "Central gathering point",
    "latitude": 50.0755,
    "longitude": 14.4378,
    "num_of_points": 10,
    "translations": [
      {
        ...
        "description": "Hlavní sraz"
      },
      {
        "language": "de",
        "title": "Hauptplatz",
        "description": "Zentraler Treffpunkt"
      }
    ]
  },
  {
